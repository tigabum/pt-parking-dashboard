    ...
    <DetailLayout
      ...
      actions={
        <div className="flex items-center gap-3">
          <Badge className="h-10 px-5 rounded flex items-center justify-center font-black uppercase text-[10px] tracking-widest bg-green-500 text-white border-none">
            Verified Asset
          </Badge>
        </div>
      }
    >
      <DetailSection title="Vehicle Detail">
        <DetailItem
          label="Plate Number"
          value={
        ...
        <DetailItem
          label="Asset Hash"
          value={vehicle.id.substring(0, 12).toUpperCase()}
        />
        <DetailItem
          label="Registry Date"
          value={dayjs(vehicle.createdAt).format("MMM D, YYYY")}
        />
        ...
        <div className="col-span-full mt-6">
          ...
          <div className="flex items-center gap-5 p-2 bg-slate-50 rounded-2xl border border-slate-100">
            <div className="h-20 w-20 rounded bg-slate-900 border-2 border-slate-100 shadow-xl overflow-hidden shrink-0 flex items-center justify-center">
              {vehicle.customer?.profileImage ? (
                <img
                  src={getImageUrl(vehicle.customer.profileImage)}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-lg font-black text-slate-900 leading-tight truncate">
                {vehicle.customer?.fullName || "Private Entity"}
              </p>
              <p className="text-xs text-slate-400 font-medium">
                {vehicle.customer?.phoneNumber || "No Contact"}
              </p>
            </div>
            <Button
              variant="ghost"
              ...
              onClick={() =>
                vehicle.customer?.id &&
                router.push(`/dashboard/customers/${vehicle.customer.id}`)
              }
              className="bg-white hover:bg-slate-50 text-slate-900 rounded h-10 px-4 font-bold transition-all text-[10px] uppercase tracking-widest border border-slate-200 shadow-sm mr-4"
            >
              Profile
            </Button>
          </div>
      ...
      <DetailSection title="Recent Movement">
            ...
            recentBookings.map((b) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-black text-primary text-sm leading-none mb-1">
                    {Number(b.totalAmount).toLocaleString()} ETB
                  </p>
