    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 font-bold text-slate-400">
                ...
                <p className="uppercase tracking-widest text-[10px]">Synchronizing Identity Data...</p>
            </div>
        );
    }
        ...
        <DetailLayout
            ...
            actions={
                <div className="flex items-center gap-3">
                    ...
                    <Badge
                        className={`h-10 px-5 rounded-xl flex items-center justify-center font-black uppercase text-[10px] tracking-widest border-none ${customer.isActive ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-700"}`}
                    >
        ...
        >
            <DetailSection title="Customer Detail">
                    ...
                    <div className="relative aspect-square w-48 rounded-[2.5rem] overflow-hidden border-2 border-slate-100 shadow-xl bg-slate-50 mx-auto lg:mx-0 p-1">
                        <Avatar className="h-full w-full rounded-[2.2rem]">
                            ...
                            <AvatarFallback className="bg-primary/5 text-primary text-4xl font-black">
                                {customer.fullName?.split(" ").map((n) => n[0]).join("") || <User className="h-16 w-16" />}
                            </AvatarFallback>
                        </Avatar>
                    </div>
                </div>
                ...
                <div className="col-span-1 md:col-span-2 lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                    ...
                    <DetailItem label="Customer Role" value={<Badge variant="outline" className="uppercase tracking-wider font-bold">Standard User</Badge>} />

                    <DetailItem label="Email Address" value={
                        <div className="flex items-center gap-2">
                            <Mail className="h-4 w-4 text-slate-400" />
                            {customer.email || "—"}
                            {customer.isEmailVerified && <CheckCircle2 className="h-3 w-3 text-green-500" />}
                        </div>
                    } />
                    <DetailItem label="Phone Number" value={
                        <div className="flex items-center gap-2">
                            ...
                            {customer.isPhoneVerified && <CheckCircle2 className="h-3 w-3 text-green-500" />}
                        </div>
                    } />
                </div>
                ...
                <DetailItem label="Last Updated" value={formatDateTime(customer.updatedAt)} />
            </DetailSection>
                    ...
                    <div className="space-y-4 pt-2">
                        {customer.vehicles && customer.vehicles.length > 0 ? (
                            customer.vehicles.map((v: any) => (
                                <div key={v.id} onClick={() => router.push(`/dashboard/vehicles/${v.id}`)} className="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-100 shadow-sm cursor-pointer hover:border-primary/30 hover:shadow-md transition-all">
                                    <div className="flex items-center gap-4">
                                        <div className="h-12 w-12 rounded-xl bg-slate-900 text-white flex items-center justify-center">
                                            <CarIcon className="h-6 w-6" />
                                        </div>
                                        <div>
                                            <p className="font-black text-slate-900 leading-tight uppercase tracking-wide">{v.plateNumber}</p>
                                            <p className="text-xs font-bold text-slate-400 mt-0.5">{v.brand} {v.name || v.model}</p>
                                        </div>
                                    </div>
                            ...
                            <div className="py-12 flex flex-col items-center justify-center gap-3 text-slate-300">
                                ...
                                <p className="text-[10px] font-bold uppercase tracking-widest leading-none">No registered vehicles</p>
                            </div>
                        )}
                    </div>
                </DetailSection>
                    ...
                    <div className="space-y-4 pt-2">
                        {recentBookings.length > 0 ? (
                            recentBookings.map((b) => (
                                <div key={b.id} onClick={() => router.push(`/dashboard/bookings/${b.id}`)} className="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-100 shadow-sm cursor-pointer hover:border-primary/30 hover:shadow-md transition-all">
                                    <div className="flex items-center gap-3">
                                        <div className="h-10 w-10 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400">
                                            <Clock className="h-5 w-5" />
                                        </div>
                                        <div>
                                            <p className="font-bold text-slate-900 text-sm leading-none">{b.plateNumber}</p>
                                            <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase">Ref: {b.referenceNo}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <Badge className={`mb-1 uppercase text-[8px] font-black tracking-widest ${b.status === 'PAID' ? 'bg-green-500' : 'bg-amber-500'}`}>
                                            {b.status}
                                        </Badge>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5">{dayjs(b.startTime).format("MMM D, HH:mm")}</p>
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="py-12 flex flex-col items-center justify-center gap-3 text-slate-300">
                                ...
                                <p className="text-[10px] font-bold uppercase tracking-widest leading-none">No transactions recorded</p>
                            </div>
                        )}
                    </div>
                </DetailSection>
