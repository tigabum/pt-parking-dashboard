...
export default function ConfigurationsPage() {
  ...
  const handleEdit = (id: string) =>
    router.push(`/dashboard/configurations/${id}/edit`);
  const handleView = (id: string) =>
    router.push(`/dashboard/configurations/${id}`);
  ...
  if (!hasPermission(PERMISSIONS.CONFIGURATION_VIEW)) {
    return (
      <div className="p-6 text-center font-bold text-red-500">
        Access Denied: Missing CONFIGURATION_VIEW permission
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6">
      ...
      >
        ...
        {hasPermission(PERMISSIONS.CONFIGURATION_CREATE) && (
          <Button
            onClick={handleAdd}
            className="h-11 rounded px-6 bg-primary hover:opacity-90 font-bold shadow-lg shadow-primary/20 text-white"
          >
