  ...
  return (
      ...
      actions={
        <div className="flex items-center gap-3">
          <Button
            ...
            className="h-10 px-4 rounded bg-primary hover:bg-primary/90 text-white font-bold text-[10px] uppercase tracking-widest transition-all shadow shadow-primary/10"
          >
            <Edit3 className="h-4 w-4 mr-2" />
            Edit Rule
          </Button>
          <Badge
            className={`h-10 px-5 rounded flex items-center justify-center font-black uppercase text-[10px] tracking-widest border-none ${commission.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
          >
    ...
    >
      <DetailSection title="Configuration Detail">
          ...
          value={
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 rounded bg-primary/5 flex items-center justify-center">
                ...
                {commission.type === CommissionType.FLAT && (
        ...
        <DetailItem
          label="Status"
          value={commission.isActive ? "Active Configuration" : "Restricted"}
        />
        <DetailItem
          label="VAT Inclusion"
          value={commission.includeVAT ? "Yes (15% Integrated)" : "Excluded"}
        />
        <DetailItem
          label="Registration Date"
          value={new Date(commission.createdAt).toLocaleDateString()}
        />
          ...
          <DetailItem
            ...
            value={
              <span className="text-2xl font-black text-primary">
                {commission.value}
                <span className="text-sm text-slate-400 ml-1 uppercase">
          ...
          <div className="col-span-full mt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {commission.tierConfig?.map((tier, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-4 rounded bg-slate-50 border border-slate-100"
                >
                  <span className="font-bold text-slate-600 text-xs">
                    {tier.minAmount} —{" "}
                    {tier.maxAmount === -1 ? "∞" : tier.maxAmount} ETB
                  </span>
        ...
        {commission.aboveThreshold > 0 && commission.aboveCommission > 0 && (
          <div className="col-span-full mt-6 p-6 rounded bg-indigo-50 border border-indigo-100 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-black uppercase text-indigo-400 tracking-widest">
                Global Overflow Surcharge
              </p>
              <p className="text-xl font-black text-indigo-900 mt-1">
                +{commission.aboveCommission} ETB Extra
              </p>
              <p className="text-[10px] font-bold text-indigo-300 mt-1">
                Applied when booking total exceeds {commission.aboveThreshold}{" "}
                ETB
              </p>
            </div>
            <div className="h-12 w-12 rounded bg-white flex items-center justify-center shadow">
