...
export default function WithholdingReceiptPage() {
    ...
    const handleGenerate = async (invoice: any) => {
        ...
        const t = toast.loading("Generating withholding receipt...");
        try {
            await invoiceService.generateWithholdingReceipt(invoice.id);
            toast.success("Withholding receipt generated successfully", { id: t });
            loadInvoices();
        } catch (err: any) {
            toast.error(err.response?.data?.message || "Withholding receipt generation failed", { id: t });
        } finally {
            setGenerating(null);
        }
    };
    ...
    return (
        <div className="p-6 space-y-6 animate-in fade-in duration-500">
            <PageHeader
                ...
                description="View invoices that include income withholding tax amounts per MOR regulations."
            >
            ...
            {/* Info banner */}
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex items-start gap-3 text-indigo-700">
                <Info className="h-5 w-5 mt-0.5 shrink-0" />
                <div className="text-sm font-medium">
                    <strong>Withholding Tax Rule:</strong> Income withholding (2%) is applied when the pre-tax invoice total exceeds ETB 10,000 (B2B transactions per MOR CORE regulations).
                </div>
            </div>
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={12} className="py-12 text-center text-sm text-slate-400 italic">
                                        No withholding receipts found (invoices over ETB 10,000 will appear here)
                                    </td>
                                </tr>
                            ) : (
                                filtered.map((inv, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3">
                                                <span className={`text-[11px] font-bold ${inv.status === "REGISTERED" ? "text-emerald-600" :
                                                    inv.status === "VERIFIED" ? "text-indigo-600" :
                                                        inv.status === "CANCELLED" ? "text-rose-600" :
                                                            inv.status === "WITHHOLDING_GENERATED" ? "text-blue-600" :
                                                                "text-slate-500"
                                                    }`}>
                                                    {inv.status}
                                                </span>
                                            ...
                                            <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                                                <Button
                                                    ...
                                                    disabled={generating === inv.id || inv.status === 'WITHHOLDING_GENERATED'}
                                                ...
                                                >
                                                    ...
                                                    {inv.status === 'WITHHOLDING_GENERATED' ? 'Generated' : 'Generate'}
                                                </Button>
                                            </td>
                                        </tr>
                                    );
            ...
            {filtered.length > 0 && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        ...
                        {
                            label: "Total Withhold",
                            value: `ETB ${filtered.reduce((sum, inv) => sum + Number(inv.valueDetails?.IncomeWithholdValue ?? 0), 0).toLocaleString()}`,
                            color: "text-orange-600",
                        },
                        {
                            label: "Total Tax",
                            value: `ETB ${filtered.reduce((sum, inv) => sum + Number(inv.taxAmount ?? 0), 0).toLocaleString()}`,
                            color: "text-indigo-600",
                        },
                        {
                            label: "Total Invoice Value",
                            value: `ETB ${filtered.reduce((sum, inv) => sum + Number(inv.totalAmount ?? 0), 0).toLocaleString()}`,
