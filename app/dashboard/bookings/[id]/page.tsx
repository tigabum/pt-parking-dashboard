...
export default function BookingDetailPage() {
    ...
    const updateTimer = () => {
      ...
      if (booking?.totalDurationMinutes) {
        ...
        if (
          ...
          (booking.status as any) === "COMPLETED" ||
  ...
  const handleConfirmPayment = async () => {
    ...
    if (
      !confirm("Are you sure you want to confirm the payment for this booking?")
    )
      return;
  ...
  const handleConfirmArrival = async () => {
    if (!id || !booking) return;
    if (!confirm("Confirm arrival for this booking? Session will start now."))
      return;
  ...
  const handleCheckout = async () => {
    if (!id || !booking) return;
    if (!confirm("Confirm checkout and finalize payment for this session?"))
      return;
  ...
  const handleCancel = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to cancel booking ${booking.referenceNo}?`,
      )
    )
      return;
  ...
  const formatMoney = (amount?: number | string | null) =>
    amount != null
      ? `${Number(amount).toLocaleString("en-US", { minimumFractionDigits: 2 })} ETB`
      : "—";
        ...
        <div className="flex items-center gap-3">
          {canExtend &&
            (booking.status === "PENDING" || booking.status === "ACTIVE") && (
              <Button
                ...
                className="h-10 px-4 rounded bg-primary hover:bg-primary/90 text-white font-bold text-[10px] uppercase tracking-widest transition-all"
              >
          ...
          {canExtend && booking.status === "PENDING" && (
            <Button
              ...
              className="h-10 px-4 rounded bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[10px] uppercase tracking-widest transition-all"
            >
              {isActionInProgress ? (
          ...
          {canExtend && booking.status === "ACTIVE" && (
            <Button
              ...
              disabled={isActionInProgress}
              className="h-10 px-4 rounded bg-orange-600 hover:bg-orange-500 text-white font-bold text-[10px] uppercase tracking-widest transition-all"
            >
              {isActionInProgress ? (
                <Loader2 className="h-4 w-4 animate-spin mr-2" />
              ) : (
          ...
          {canExtend &&
            (booking.status === "PENDING" ||
              ...
              <Button
                ...
                disabled={isActionInProgress}
                className="h-10 px-4 rounded bg-green-600 hover:bg-green-500 text-white font-bold text-[10px] uppercase tracking-widest transition-all"
              >
                {isActionInProgress ? (
                  <Loader2 className="h-4 w-4 animate-spin mr-2" />
                ) : (
          ...
          {canExtend && booking.status === "PENDING" && (
            <Button
              ...
              disabled={isActionInProgress}
              variant="destructive"
              className="h-10 px-4 rounded font-bold text-[10px] uppercase tracking-widest transition-all"
            >
              {isActionInProgress ? (
                <Loader2 className="h-4 w-4 animate-spin mr-2" />
              ) : (
          ...
          <Badge
            className={cn(
              (booking.status as any) === "PAID"
                ...
                : (booking.status as any) === "WAITING_CONFIRMATION"
                  ? "bg-amber-500 shadow-amber-100 text-white"
                  : (booking.status as any) === "ACTIVE"
                    ? "bg-blue-600 shadow-blue-100 text-white"
                    : (booking.status as any) === "CANCELLED"
                      ? "bg-red-500 shadow-red-100 text-white"
                      : (booking.status as any) === "EXPIRED"
                        ? "bg-slate-500 shadow-slate-100 text-white"
                        : (booking.status as any) === "REFUNDED"
    ...
    >
      <DetailSection title="Booking Detail">
        ...
        <DetailItem
          ...
          value={
            booking.endTime ? formatDateTime(booking.endTime) : "Open Ended"
          }
        />
        {booking.type && (
          <DetailItem
        ...
        <DetailItem
          ...
          value={
            ...
            >
              {booking.status === "PAID" ? "SETTLED" : "OUTSTANDING"}
            </Badge>
          }
        />
        ...
        <DetailItem
          label="Created At"
          value={dayjs(booking.createdAt).format("MMM D, YYYY HH:mm")}
        />
        <DetailItem
          label="Last Change"
          value={dayjs(booking.updatedAt).format("MMM D, YYYY HH:mm")}
        />
        {booking.createdBy && (
          <DetailItem
            label="Originator"
            ...
            <div className="col-span-full space-y-2 mt-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {parking.galleryImages.map((img, i) => (
                  <div
                    key={i}
                    className="aspect-square rounded overflow-hidden border border-slate-100 bg-slate-50 group relative shadow-sm"
                  >
                    <img
                      src={getImageUrl(img)}
                      alt={`Gallery ${i}`}
      ...
      <Dialog open={isExtendOpen} onOpenChange={setIsExtendOpen}>
        <DialogContent className="sm:max-w-md rounded-[2rem] p-0 overflow-hidden border-none shadow-2xl">
          <div className="bg-primary p-8 text-white">
            <DialogHeader>
              <DialogTitle className="text-2xl font-black tracking-tight uppercase tracking-[0.1em]">
                Extend Session
              </DialogTitle>
            </DialogHeader>
            <p className="text-white/40 font-medium text-[10px] uppercase tracking-widest mt-2">
          ...
          <div className="p-8 space-y-8">
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-12 rounded border-slate-200 text-sm font-bold focus:ring-primary/20 transition-all"
                />
              </div>
            ...
            <div className="flex flex-col gap-2 pt-4">
              <Button
                ...
                className="h-12 rounded bg-primary hover:bg-primary/90 text-white font-black uppercase text-[10px] tracking-[0.2em] shadow-lg shadow-primary/20 transition-all active:scale-95"
              >
                {extending ? (
              ...
              <Button
                ...
                className="h-10 rounded font-bold uppercase text-[10px] tracking-widest text-slate-400 hover:bg-slate-50"
              >
                Abandon
              </Button>
            </div>
