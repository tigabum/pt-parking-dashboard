...
export default function ParkingUsersPage() {
  ...
  const handleEditUser = (user: User) =>
    router.push(`/dashboard/parking-users/${user.id}/edit`);
  const handleDetailUser = (user: User) =>
    router.push(`/dashboard/parking-users/${user.id}`);
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between w-full gap-4 mt-2">
          ...
          <div className="flex flex-wrap items-center gap-3 flex-1 lg:justify-center">
            <Select value={roleFilter} onValueChange={setRoleFilter}>
              <SelectTrigger className="w-[160px] h-11 rounded border-slate-200 bg-white">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="w-[140px] h-11 rounded border-slate-200 bg-white">
          ...
          {hasPermission(PERMISSIONS.USER_CREATE) && (
            <Button
              ...
              className="h-11 rounded px-6 bg-primary hover:opacity-90 font-bold shadow-lg shadow-primary/20 shrink-0"
            >
