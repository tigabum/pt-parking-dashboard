    ...
    const handleTopUp = async () => {
        ...
        try {
            ...
            const response = await walletService.addFunds(
                parkingId,
                amount,
                topUpDescription || (user?.role === UserRole.PARKING_SUPER_ADMIN ? "Parking Admin Top-up" : "System Admin Top-up")
            );
    ...
    const columns: Column<any>[] = [
        {
            ...
            render: (row) => (
                <div className="flex flex-col">
                    <span className="font-bold text-slate-900">{row.parking?.name || "—"}</span>
                    <span className="text-[10px] text-slate-400 font-mono tracking-wider">{row.parking?.parkingCode || "—"}</span>
                </div>
            ),
        },
        {
            ...
            header: "Balance",
            render: (row) => (
                <span className="text-lg font-black text-slate-900">{Number(row.balance).toLocaleString()}</span>
            ),
        },
        {
            key: "currency",
            header: "Currency",
            render: (row) => (
                <Badge variant="outline" className="font-bold text-slate-500 bg-slate-50 border-slate-200">
                    {row.currency || "ETB"}
                </Badge>
            ),
        },
            ...
            <Dialog open={isTopUpOpen} onOpenChange={setIsTopUpOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Add funds to <b>{selectedWallet?.parking?.name}</b> wallet manually.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 font-bold text-sm">ETB</span>
                                <Input
                                    id="amount"
                                    type="number"
                                    placeholder="0.00"
