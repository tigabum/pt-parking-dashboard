    ...
    return (
        <div className="flex flex-col sm:flex-row items-center justify-between py-6 gap-4">
            <div className="flex items-center gap-2 order-2 sm:order-1">
                <span className="text-[12px] md:text-[13px] text-slate-600 font-medium">Rows per page:</span>
                <Select
                    value={String(limit)}
                ...
                <span className="text-[11px] text-slate-400 font-bold ml-2 hidden sm:inline">Total: {total}</span>
            </div>
                    ...
                    {Array.from({ length: totalPages }).map((_, i) => {
                        ...
                        if (totalPages > 5) {
                            ...
                            if (isMobile) {
                                ...
                                if (pageNum > 1 && pageNum < totalPages && Math.abs(pageNum - page) > 1) {
                                    if (pageNum === 2 && page > 3) return <span key="start-dots" className="px-1 text-slate-400 text-xs">...</span>;
                                    if (pageNum === totalPages - 1 && page < totalPages - 2) return <span key="end-dots" className="px-1 text-slate-400 text-xs">...</span>;
                        ...
                        return (
                            <Button
                                ...
                                size="sm"
                                className={`h-8 w-8 p-0 text-[12px] md:text-[13px] font-bold transition-all rounded-[10px] ${page === pageNum
