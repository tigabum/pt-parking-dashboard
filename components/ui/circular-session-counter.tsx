...
export function CircularSessionCounter({ startTime, endTime, displayTime, stopped = false, size = 288 }: CircularSessionCounterProps) {
    ...
    const radius = size * 0.31; // Dynamic radius based on size
    const center = size / 2;
    ...
    return (
        ...
        >
            ...
            <svg
                ...
                className="transform -rotate-90 relative z-10 drop-shadow-[0_0_15px_rgba(0,102,255,0.1)]"
            >
                <defs>
            ...
            <div className="absolute text-center z-20 flex flex-col items-center justify-center p-4">
                <div className="flex items-center justify-center gap-2 mb-2">
                    <div className="h-5 w-5 rounded-full bg-[#0066FF]/10 flex items-center justify-center shrink-0">
                        <Zap className="h-2.5 w-2.5 text-[#0066FF] fill-[#0066FF]" />
                    </div>
                    <span className="text-[9px] font-black uppercase text-slate-400 tracking-[0.2em]">Live Session</span>
                </div>

                <div
                    className="font-mono font-black text-slate-900 tracking-tighter tabular-nums leading-none"
                    style={{ fontSize: size * 0.18 }} // Responsive font size
                >
                    {liveTime}
                </div>

                <div className="mt-4 sm:mt-6">
                    <div className="px-3 py-1 bg-slate-900 rounded-full flex items-center gap-2 shadow-xl shadow-slate-200">
                        <div className="h-1 w-1 rounded-full bg-[#0066FF] animate-pulse" />
                        <p
                            className="font-black text-white uppercase tracking-widest"
                            style={{ fontSize: Math.max(7, size * 0.03) }}
                        >
