...
export function BookingForm({
  ...
  const [institutions, setInstitutions] = useState<FinancialInstitution[]>([
    {
      ...
      name: "Telebirr",
      type: "WALLET",
      checkoutUrl: "https://www.ethio-telebirr.et/",
    },
  ...
  const [booking, setBooking] = useState<CreateBooking>({
    ...
    endTime:
      initialData?.endTime || dayjs().add(1, "hour").format("YYYY-MM-DDTHH:mm"),
    ...
    const timer = setInterval(() => {
      if (booking.startTime) {
        ...
        if (diff.asMilliseconds() > 0) {
          setSessionDuration(
            `${Math.floor(diff.asHours()).toString().padStart(2, "0")}:${diff
              .minutes()
              .toString()
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setBooking((prev) => ({
        ...prev,
        parkingId: (parkings[0] as any).id || "",
      }));
    }
  ...
  const validateStep2 = () => {
    ...
    if (!booking.customerName?.trim())
      newErrors.customerName = "Driver name is required";
    if (!booking.plateNumber?.trim())
  ...
  const handleEndParking = async () => {
    ...
    try {
      ...
      if (createdBooking.status === "WAITING_CONFIRMATION") {
        ...
        if (res.success) {
          toast.success("Session Finalized & Release Authorized", {
            id: loadingToast,
          });
          onOpenChange(false);
          return;
      ...
      if (response) {
        toast.success("Session Ended. Awaiting Manager Confirmation.", {
          id: loadingToast,
        });
        onOpenChange(false);
      }
  ...
  const validateStep0 = () => {
    const newErrors: Record<string, string> = {};
    if (!booking.parkingId && (!parkings || parkings.length === 0))
      newErrors.parkingId = "Please select an assigned parking";
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (
        ...
        (cRes.value.success || cRes.value.id)
      ) {
      ...
      if (
        ...
        (vRes.value.success || vRes.value.id)
      ) {
      ...
      const updatedName =
        ...
        foundVehicle?.customer?.fullName ||
        booking.customerName;
      const updatedPhone =
        foundCustomer?.phoneNumber ||
        foundVehicle?.customer?.phoneNumber ||
        booking.customerPhone;
      const updatedBrand = foundVehicle?.brand || booking.vehicleBrand;
      const updatedModel =
        foundVehicle?.model || foundVehicle?.name || booking.vehicleName;
      ...
      if (foundCustomer || foundVehicle) {
        ...
        if (
          foundCustomer &&
          foundVehicle &&
          foundVehicle.customer?.id === foundCustomer.id
        ) {
    ...
    try {
      ...
      if (customer && customer.id) {
        ...
        setBooking((prev: CreateBooking) => ({
          ...
          vehicleName:
            prev.vehicleName || mainVehicle?.model || mainVehicle?.name || "",
        }));
        setCustomerFound(true);
  ...
  const searchVehicle_action = async (plate: string) => {
    ...
    try {
      ...
      if (vehicle && vehicle.id) {
        ...
        toast.success(
          `Vehicle found: ${vehicle.brand} ${vehicle.model || vehicle.name || ""}`,
        );
      } else {
  ...
  const getSelectedParkingName = () => {
    return (
      parkings.find((p) => p.id === booking.parkingId)?.name ||
      "Unknown Parking"
    );
  };
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 pt-12 md:pt-24">
        <div className="max-w-250 mx-auto pb-20">
          <div className="bg-white p-6 md:p-8 rounded-2xl border shadow-sm space-y-8">
            {step === 0 && (
              <div className="space-y-8 min-h-137.5 flex flex-col justify-center animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="w-full space-y-12">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 font-bold">
                    {/* Parking Name */}
                    <div className="space-y-3">
                      <Label className="text-[11px] uppercase font-black text-slate-400 tracking-[0.2em]">
                        Parking Name
                      </Label>
                      <Input
                        readOnly
                        value={
                          parkings.find((s) => s.id === booking.parkingId)
                  ...
                  <div className="flex justify-end gap-3 pt-8">
                    ...
                    <Button
                      onClick={handleNext}
                      disabled={searching}
                      className="h-14 px-14 rounded-2xl bg-primary text-white font-bold flex items-center gap-3 active:scale-95 transition-all text-lg shadow-none border-none"
                    >
                      {searching ? (
              ...
              <div className="space-y-8 min-h-137.5 flex flex-col justify-between animate-in fade-in slide-in-from-right-4 duration-500 font-bold">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2 col-span-1 md:col-span-2">
                    ...
                    <Input
                      ...
                      className={`h-12 rounded bg-slate-50 border-none font-bold ${errors.customerName ? "border-2 border-red-500 bg-red-50" : ""} ${customerFound ? "bg-emerald-50 text-emerald-900" : ""}`}
                    />
                    {errors.customerName && (
                      <p className="text-xs text-red-500 font-medium">
                        {errors.customerName}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className={`h-12 rounded bg-slate-50 border-none font-bold ${vehicleFound ? "bg-emerald-50 text-emerald-900" : ""}`}
                    />
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v: BookingType) =>
                        setBooking({ ...booking, type: v })
                      }
                    >
                      <SelectTrigger className="w-full h-12 rounded bg-slate-50 border-none font-bold">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="rounded border-none shadow-2xl">
                        <SelectItem
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={
                        booking.paymentMethod === PaymentMethod.INCASH
                          ? "INCASH"
                          : "TRANSFER"
                      }
                      onValueChange={(v) => {
                    ...
                    >
                      ...
                      <SelectContent className="rounded border-none shadow-2xl">
                        <SelectItem value="INCASH" className="font-bold">
                          Cash
                        </SelectItem>
                ...
                <div className="flex justify-between items-center gap-4 pt-6 border-t">
                  <Button
                    variant="ghost"
                    onClick={() => setStep(0)}
                    className="h-12 px-6 rounded font-bold text-slate-400 flex items-center gap-2"
                  >
                  ...
                  <div className="flex gap-3">
                    <Button
                      variant="ghost"
                      onClick={() => onOpenChange(false)}
                      className="h-12 px-8 rounded font-bold text-slate-400"
                    >
                      Cancel
                    </Button>
                    <Button
                      onClick={() => setStep(2)}
                      className="h-12 px-10 rounded bg-primary text-white font-bold flex items-center gap-3 shadow-none border-none"
                    >
                      Review & Proceed
            ...
            {step === 2 && (
              <div className="space-y-10 min-h-137.5 flex flex-col justify-between animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="space-y-8">
                  <div className="text-center space-y-2">
                    <div className="h-20 w-20 bg-primary/5 rounded-4xl flex items-center justify-center mx-auto text-primary mb-6">
                    ...
                    <p className="text-slate-500 font-medium text-sm">
                      Review the session parameters before verification.
                    </p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="p-6 bg-slate-50 rounded space-y-2 border border-slate-100">
                      <Label className="text-[10px] uppercase font-black text-slate-400 tracking-widest">
                        Driver Details
                      </Label>
                    ...
                    <div className="p-6 bg-slate-50 rounded space-y-2 border border-slate-100">
                      ...
                      <p className="text-slate-900 font-bold text-base uppercase font-mono tracking-widest">
                        {booking.plateNumber}
                      </p>
                      <p className="text-slate-500 text-xs font-medium">
                    ...
                    <div className="p-6 bg-slate-50 rounded space-y-2 border border-slate-100">
                      ...
                      <p className="text-slate-500 text-xs font-medium">
                        Until {dayjs(booking.endTime).format("HH:mm")}
                      </p>
                    </div>
                  </div>

                  <div className="flex flex-col items-center justify-center p-8 bg-slate-50/50 border-2 border-slate-100 rounded-lg space-y-2">
                    <Label className="text-[10px] uppercase font-bold text-slate-400 tracking-[0.4em]">
                      Est. Total Amount
                    </Label>
                    <div className="flex items-baseline gap-2">
                      <span className="text-4xl font-bold text-slate-900 tabular-nums tracking-tighter">
                        {getPriceEstimate()}
                      </span>
                ...
                <div className="flex justify-between items-center pt-8 border-t border-slate-100">
                  ...
                  {(() => {
                    ...
                    const now = dayjs();
                    const monthReached =
                      now.isSame(startDate, "month") ||
                      now.isAfter(startDate, "month");
                      ...
                      return (
                        <div className="flex flex-col items-end gap-2">
                          <p className="text-xs font-bold text-amber-600 uppercase tracking-wider">
                            Monthly booking confirmation available from{" "}
                            {startDate.format("MMMM YYYY")}
                          </p>
                          <Button
                            disabled
                            className="h-16 px-16 rounded-4xl bg-slate-200 text-slate-400 font-black flex items-center gap-4 text-lg uppercase tracking-widest shadow-none border-none cursor-not-allowed"
                          >
                    ...
                    return (
                      <Button
                        ...
                        disabled={isLoading}
                        className="h-16 px-16 rounded-4xl bg-primary text-white font-black transition-all active:scale-95 flex items-center gap-4 text-lg uppercase tracking-widest shadow-none border-none"
                      >
                        {isLoading ? (
            ...
            {step === 3 && createdBooking && (
              <div className="space-y-12 min-h-137.5 flex flex-col items-center justify-center animate-in fade-in duration-700 font-bold">
                <div className="text-center space-y-2">
                ...
                <div className="w-full max-w-md grid grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-center">
                    <p className="text-[8px] uppercase font-black text-slate-400 tracking-widest">
                      Plate ID
                    </p>
                    <p className="text-slate-900 font-black tracking-widest uppercase">
                      {booking.plateNumber}
                    </p>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-center">
                ...
                <div className="w-full max-w-md flex flex-col gap-4">
                  <Button
                    ...
                    className="w-full h-16 rounded-4xl bg-red-500 hover:bg-black text-white font-bold text-lg shadow-xl transition-all active:scale-95 uppercase tracking-[0.15em] border-none"
                  >
                    {createdBooking.status === "WAITING_CONFIRMATION"
