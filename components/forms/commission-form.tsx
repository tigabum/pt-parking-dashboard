  ...
  const addTier = () => {
    ...
    if (lastTier) {
      ...
      if (
        ...
        lastTier.maxAmount <= lastTier.minAmount
      ) {
    ...
    if (form.type === CommissionType.TIER) {
      ...
      for (let i = 0; i < form.tierConfig.length; i++) {
        ...
        if (tier.maxAmount !== -1 && tier.maxAmount <= tier.minAmount) {
          toast.error(
            `Tier ${i + 1}: Max amount must be greater than min amount`,
          );
          setErrors(newErrors);
          return false;
        }
      ...
      if ((form.aboveThreshold ?? 0) > 0) {
        if (lastTier.maxAmount === -1) {
          toast.error(
            "Final tier cannot be infinity if a threshold surcharge is defined. Please set a max value for the last tier.",
          );
          setErrors(newErrors);
          return false;
        }
        if ((form.aboveThreshold ?? 0) <= lastTier.maxAmount) {
          toast.error(
            `Threshold value must be greater than the final tier's max amount (${lastTier.maxAmount} ETB)`,
          );
          setErrors(newErrors);
          return false;
        }
  ...
  return (
    <div className="w-full h-full bg-slate-50 overflow-y-auto">
      <div className="min-h-full w-full flex flex-col items-center py-12 px-6">
        <div className="w-full max-w-212.5 bg-white rounded-lg shadow-2xl shadow-slate-200/50 border border-slate-100 p-12 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {/* Visual Header */}
          ...
          <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-3">
                ...
                <Select
                  value={form.type}
                  onValueChange={(val) =>
                    setForm({ ...form, type: val as CommissionType })
                  }
                >
                  <SelectTrigger className="h-12 w-full rounded border-slate-200 bg-slate-50/50 font-bold focus:ring-primary">
                  ...
                  <SelectContent className="rounded">
                    ...
                    <SelectItem value={CommissionType.FLAT}>
                      Flat Fee (ETB)
                    </SelectItem>
                    <SelectItem value={CommissionType.TIER}>
                      Tiered Brackets
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-3">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setForm({
                      ...form,
                      parkingId: val === "global" ? undefined : val,
                    })
                  }
                >
                  <SelectTrigger className="h-12 w-full rounded border-slate-200 bg-slate-50/50 font-bold focus:ring-primary">
                  ...
                  <SelectContent className="rounded max-h-75">
                    <SelectItem
                      value="global"
              ...
              {form.type !== CommissionType.TIER && (
                <div className="space-y-3">
                  <Label className="text-sm font-bold text-slate-700 uppercase tracking-widest text-[10px]">
                    {form.type === CommissionType.PERCENTAGE
                      ? "Percentage Value (%)"
                      : "Flat Amount (ETB)"}
                    *
                  </Label>
            ...
            {/* VAT & TAX SECTION */}
            <div className="p-8 rounded bg-primary/5 border border-primary/10 flex items-center justify-between relative transition-all hover:bg-primary/10">
              ...
              <div className="space-y-8 pt-4">
                ...
                <div className="space-y-8">
                  {form.tierConfig?.map((tier, i) => (
                    <div
                      key={i}
                      className="group relative animate-in fade-in slide-in-from-left-2 duration-300"
                    >
                          ...
                          <div className="flex gap-3">
                            ...
                            {form.tierConfig!.length > 1 && (
                              <Button
                                ...
                                className="h-14 w-14 shrink-0 rounded text-slate-300 hover:text-red-500 hover:bg-red-50 transition-colors"
                              >
                  ...
                  <div className="flex justify-end">
                    <Button
                      ...
                      variant="outline"
                      onClick={addTier}
                      className="h-12 px-8 rounded border-primary text-primary font-black hover:bg-primary/5 border-2"
                    >
            ...
            {form.type === CommissionType.TIER && (
              <div className="space-y-8 pt-8 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-2">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      />
                      <span className="absolute right-6 top-1/2 -translate-y-1/2 font-bold text-primary/30">
                        ETB
                      </span>
                    </div>
                  </div>
                ...
                <p className="text-[11px] font-medium text-slate-400 italic">
                  This surcharge applies to any booking total exceeding the
                  specified threshold, after base tiers are calculated.
                </p>
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-4 pt-12 border-t border-slate-100">
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className="h-14 px-12 rounded bg-primary text-white font-bold transition-all flex items-center justify-center gap-3 min-w-45 shadow-none border-none"
              >
                {isLoading ? (
