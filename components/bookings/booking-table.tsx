...
import { ReusableTable, statusColumn, Column } from "@/components/tables";
...
function ConfirmationPopup({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="w-80 p-5 rounded shadow-2xl border-slate-100 animate-in zoom-in-95 duration-200"
        align="end"
        side="top"
        sideOffset={10}
      >
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className="flex-1 h-9 rounded text-[10px] uppercase font-black tracking-wider"
            >
              Cancel
            </Button>
            <Button
...
function BookingActions({
  ...
  const showConfirmPayment =
    (row.status === "PENDING" || row.status === "WAITING_CONFIRMATION") &&
    hasPermission(PERMISSIONS.BOOKING_UPDATE) &&
    !isSystemAdmin;
  ...
  const showCancel =
    ...
    (hasPermission(PERMISSIONS.BOOKING_UPDATE) ||
      hasPermission(PERMISSIONS.BOOKING_DELETE));
  const showDetail = hasPermission(PERMISSIONS.BOOKING_VIEW);

  if (!showDetail && !showConfirmPayment && !showConfirmArrival && !showCancel)
    return null;
...
export function BookingsTable({
  ...
  const { user } = useAuth();
  const onDetail = (booking: BookingResponse) =>
      ...
      render: (row) => (
        <div className="flex flex-col">
          ...
          <span className="text-xs text-muted-foreground">
            {row.vehicleName || row.vehicleBrand || "—"}
          </span>
        </div>
      ),
    },
